...
@Rule(
        priority = 0,
        name = "independence day rule",
        description = "if there is a holiday charge and it is independence day, update the cost"
)
public class IndependenceDayRule {
    @Condition
    public boolean itIsIndependenceDay(@Fact("tool") Tool tool, @Fact("date") LocalDate date) {
        return LocalDateUtil.isIndependenceDay(date);
    }
